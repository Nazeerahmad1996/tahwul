import PropTypes from 'prop-types';

export default function DocumentsIcon({ size = 16, strokeWidth = 1.5, className = '', ...props }) {
    return (
        <svg
            width={size}
            height={size}
            viewBox="0 0 16 16"
            fill="none"
            xmlns="http://www.w3.org/2000/svg"
            className={className}
            {...props}
        >
            <path
                d="M2.33333 6.66671C2.33333 4.15271 2.33333 2.89537 3.16333 2.11471C3.99333 1.33404 5.32867 1.33337 8 1.33337H8.51533C10.6887 1.33337 11.7767 1.33337 12.5313 1.86537C12.7473 2.01737 12.9393 2.19871 13.1013 2.40204C13.6667 3.11271 13.6667 4.13537 13.6667 6.18204V7.87871C13.6667 9.85404 13.6667 10.842 13.354 11.6307C12.8513 12.8994 11.7887 13.8994 10.4407 14.3727C9.60267 14.6667 8.554 14.6667 6.454 14.6667C5.25533 14.6667 4.65533 14.6667 4.17667 14.4987C3.40667 14.228 2.79933 13.6567 2.512 12.932C2.33333 12.4814 2.33333 11.9167 2.33333 10.788V6.66671Z"
                stroke="currentColor"
                strokeWidth={strokeWidth}
                strokeLinecap="round"
                strokeLinejoin="round"
            />
            <path
                d="M13.6667 8.00008C13.6667 8.58939 13.4326 9.15457 13.0159 9.57127C12.5992 9.98798 12.034 10.2221 11.4447 10.2221C11.0007 10.2221 10.4773 10.1447 10.046 10.2601C9.85761 10.3107 9.68583 10.41 9.54789 10.548C9.40994 10.6859 9.31066 10.8577 9.26 11.0461C9.14467 11.4774 9.222 12.0007 9.222 12.4447C9.222 12.7365 9.16453 13.0255 9.05286 13.2951C8.94119 13.5647 8.77752 13.8096 8.57119 14.0159C8.36486 14.2223 8.11991 14.3859 7.85032 14.4976C7.58074 14.6093 7.2918 14.6667 7 14.6667M5.33333 4.66675H10M5.33333 7.33341H7.33333"
                stroke="currentColor"
                strokeWidth={strokeWidth}
                strokeLinecap="round"
                strokeLinejoin="round"
            />
        </svg>
    );
}

DocumentsIcon.propTypes = {
    size: PropTypes.number,
    strokeWidth: PropTypes.number,
    className: PropTypes.string,
};

